...
def create_minecraft_env(env_id):
    env = gym.make(env_id)
    env.init(start_minecraft=True)
    env = Vectorize(env)
    env = MinecraftRescale84x84(env)
    env = DiagnosticsInfo(env)
    env = Unvectorize(env)
    return env
...
def _process_mine84(frame):
    ...
    # # we resize directly we lose pixels that, when mapped to 42x42,
    # # aren't close enough to the pixel boundary.
    frame_R = cv2.resize(frame_R, (84, 84))
    frame_G = cv2.resize(frame_G, (84, 84))
    frame_B = cv2.resize(frame_B, (84, 84))
    frame_R = np.reshape(frame_R, [84,84,1])
    frame_G = np.reshape(frame_G, [84,84,1])
    frame_B = np.reshape(frame_B, [84,84,1])
    frame = np.concatenate((frame_B, frame_G, frame_R), axis=2)
    # frame = frame.mean(2)
    frame = frame.astype(np.float32)
    # frame *= (1.0 / 255.0)
    return frame

class MinecraftRescale84x84(vectorized.ObservationWrapper):
    def __init__(self, env=None):
        super(MinecraftRescale84x84, self).__init__(env)
        self.observation_space = Box(0.0, 1.0, [80, 80, 3])

    def _observation(self, observation_n):
        processed_obs = [_process_mine84(observation) for observation in observation_n]
