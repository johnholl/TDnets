...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = AuxLSTMPolicy(env.observation_space_shape, env.num_actions, grid_size=grid_size)
        ...
        with tf.device(worker_device):
            ...
            #     self.local_prediction_network = AuxLSTMPolicy(env.observation_space_shape, env.num_actions, grid_size=grid_size)
            #     self.local_prediction_network.global_step = self.global_step

            with tf.variable_scope("local"):
                self.local_network = pi = AuxLSTMPolicy(env.observation_space_shape, env.num_actions, grid_size=grid_size)
            ...
            with tf.variable_scope("predictions"):
                self.extra_placeholder = tf.placeholder(tf.float32, [None], name="extra_placeholder")
                self.extra_constant = tf.constant(value=7, name="extra_constant")
                self.extra_variable = tf.Variable(initial_value=.5, name="extra_variable")
