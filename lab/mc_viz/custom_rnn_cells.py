...
class BasicPixelRNN(RNNCell):
  """The most basic pixel RNN cell."""

  def __init__(self, num_units, input_size=None, activation=tanh, grid_size=20):
    if input_size is not None:
      logging.warn("%s: The input_size parameter is deprecated.", self)
    self._num_units = num_units
    self._activation = activation
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or "basic_rnn_cell"):
      z = tf.nn.l2_normalize(state, dim=1)
      z = tf.nn.tanh(linear(z, 256, 'encode_pred', normalized_columns_initializer(0.1)))
      output = x = self._activation(
          _linear([inputs, z], self._num_units, True, scope=scope))
      ...
      new_pred_shaped = tf.nn.conv2d_transpose(y, deconv_weights,
                                                output_shape=[1, self.grid_size, self.grid_size, 1],
                                                strides=[1,2,2,1], padding='SAME')
      new_pred_flat = tf.reshape(new_pred_shaped, shape=[1, self.grid_size*self.grid_size])

    return output, new_pred_flat
