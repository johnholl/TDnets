...
from model import GridLSTMPolicy
import random
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            ...
            policy.update_replay_memory((last_state, action, reward, terminal,
                                         ...
                                         state, pix_change, value_, features
                                         ))
            length += 1
            rewards += reward
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            with tf.variable_scope("global"):
                self.network = GridLSTMPolicy(env.observation_space_shape, env.num_actions)
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = GridLSTMPolicy(env.observation_space_shape, env.num_actions)
            ...
            self.agent_loss = tf.reduce_sum(tf.select(
                                    tf.abs(q_delta) < 1,
            ...
            pix_delta = prediction_readout - self.prediction_target
...
"""
        ...
        if len(self.runner.policy.replay_memory) > 100:
            ...
            secondstart_features = Qbatch[0][10]
            states = [m[7] for m in Qbatch]
            pixel_changes = [m[8] for m in Qbatch]
        ...
        # Q learning update
            ...
            feed_dict={self.local_network.x: states,
                       ...
                       self.local_network.state_in[0]: secondstart_features[0],
                       self.local_network.state_in[1]: secondstart_features[1],
                       self.local_network.state_in[2]: secondstart_features[2]
                       }
            ...
            # print(rewards)
            # print(np.sum(rewards[16:]))
            # print(np.sum(rewards[2:], axis=0))
            ...
            for i in range(len(Qbatch)):
                ...
                if not Qbatch[-1][3]:
                    pred_targets[-1] += (.99**(len(Qbatch)-i))*max_prediction_value
