...
class FFVPCR_Policy(object):
    def __init__(self, ob_space, ac_space, grid_size=20):
        ...
        p = tf.nn.l2_normalize(self.pred, dim=1)
        p = tf.nn.tanh(linear(p, 256, 'encode_pred', normalized_columns_initializer(0.1)))

        x = tf.concat(axis=1, values=[x, self.action, self.reward, p])
...
#### (5) Feedforward agent with no prediction input. Learning agent cannot resolve any partial observability
class NOPRED_Policy(object):
    def __init__(self, ob_space, ac_space, grid_size=20):
        self.x = x = tf.placeholder(tf.float32, [None] + list(ob_space))
        self.action = tf.placeholder(tf.float32, [None, ac_space])
        self.reward = tf.placeholder(tf.float32, [None, 1])
    ...
    def value(self, ob, prev_a, prev_r):
        sess = tf.get_default_session()
        reward_value, prediction_values = sess.run(self.val, {self.x: [ob], self.action: [prev_a],
                                                                                  self.reward: [[prev_r]], self.bs:1})
