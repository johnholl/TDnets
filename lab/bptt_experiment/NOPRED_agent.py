...
from model import BehaviorAgent, NOPRED_Policy
...
"""
    ...
    def add(self, state, action, reward, value, terminal, prev_action, prev_reward, pix_change, next_state):
...
"""
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            ...
            value_ = policy.value(last_state, prev_action, prev_reward)

            action_index = np.where(action==1)
            state, reward, terminal, pix_change = env.step(action_index[0][0])

            # collect the experience
            rollout.add(last_state, action, reward, value_, terminal, prev_action, [prev_reward],
                        pix_change, state)
...
"""
        ...
        with tf.device(tf.train.replica_device_setter(1, worker_device=worker_device)):
            ...
            with tf.variable_scope("learner"):
                self.network = NOPRED_Policy(env.observation_space_shape, env.num_actions)
                self.global_step = tf.get_variable("global_step", [], tf.int32, initializer=tf.constant_initializer(0, dtype=tf.int32),
                                                   trainable=False)

        with tf.device(worker_device):
            with tf.variable_scope("local"):
                self.local_network = pi = NOPRED_Policy(env.observation_space_shape, env.num_actions)
...
"""
        ...
        feed_dict = {
            ...
            self.local_network.bs: len(batch_si)-1
