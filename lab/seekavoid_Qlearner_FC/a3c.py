...
"""
        ...
        with tf.device(worker_device):
            with tf.variable_scope("local"):
                ...
                self.agent_loss = tf.reduce_sum(tf.square(Q_readout - self.Q_target))
...
"""
        ...
        if len(self.runner.policy.replay_memory) > 100:
            ...
            pixel_changes = [m[9] for m in Qbatch]
        ...
        # Q learning update
            ...
            for i in range(len(Qbatch)):
                Q_targets.append(np.sum(rewards[i:], axis=0))
                if not Qbatch[-1][3]:
                    Q_targets[-1] += values[-1]
            ...
            if should_compute_summary:
                pixfetches = [self.pixloss_sum, self.prediction_train_op, self.global_step]
            else:
